...
extern void ClearVideoMemRegion(PT_VideoMem ptVideoMem, PT_Layout ptLayout, unsigned int dwColor);


static int ManualPageGetInputEvent(PT_PageLayout ptPageLayout, PInputEvent ptInputEvent);
...
static PixelDatas g_tOriginPicPixelDatas;
static PixelDatas g_tZoomedPicPixelDatas;
...
static void  CalcManualPageMenusLayout(PT_PageLayout ptPageLayout)
{
	...
	if (ptPDispBuff->iXres < ptPDispBuff->iYres)
	{			 
		/*	 iXres/6
		 *	  --------------------------------------------------------------
...
static void CalcManualPagePictureLayout(void)
{
	...
	if (ptPDispBuff->iXres < ptPDispBuff->iYres)
	{
		 ...
		 */
		iTopLeftX  = 0;
		iBotRightX = ptPDispBuff->iXres - 1;
		iTopLeftY  = g_atManualMenuIconsLayout[0].iBotRightY + 1;
		iBotRightY = ptPDispBuff->iYres - 1;
	}
	else
	{
		/*	 iYres/6
 ...
 * 2013/02/08	     V1.0	  韦东山	      创建
 ***********************************************************************/
static PPixelDatas GetOriginPictureFilePixelDatas(char *strFileName)
{
    int iError;
 ...
 * 2013/02/08	     V1.0	  韦东山	      创建
 ***********************************************************************/
static PPixelDatas GetZoomedPicPixelDatas(PPixelDatas ptOriginPicPixelDatas, int iZoomedWidth, int iZoomedHeight)
{
    float k;
	PDispBuff ptPDispBuff = GetDisplayBuffer();
    ...
    g_tZoomedPicPixelDatas.iBpp        = ptPDispBuff->iBpp;
...
{
    PPixelDatas ptOriginPicPixelDatas;
    PPixelDatas ptZoomedPicPixelDatas;
...
static void ShowManualPage(PT_PageLayout ptPageLayout, char *strFileName)
{
	...
	if (ptVideoMem == NULL)
	{
		printf("can't get video mem for manual page!\n");
		return;
	}
 ...
 * 2013/02/08	     V1.0	  韦东山	      创建
 ***********************************************************************/
static int DistanceBetweenTwoPoint(PInputEvent ptInputEvent1, PInputEvent ptInputEvent2)
{
 ...
 * 2013/02/08	     V1.0	  韦东山	      创建
 ***********************************************************************/
static void ShowZoomedPictureInLayout(PPixelDatas ptZoomedPicPixelDatas, PT_VideoMem ptVideoMem)
{
...
static void ManualPageRun(PT_PageParams ptParentPageParams)
{
    ...
    int iZoomedWidth;
    int iZoomedHeight;
    PPixelDatas ptZoomedPicPixelDatas = &g_tZoomedPicPixelDatas;
    ...
    {
        /* 如果它的父页面是主页面 - 显示browse页面*/
        GetPages("browse")->Run(ptParentPageParams);
    }
    else
    {
...
static PageAction g_tManualPageAction = {
	.name          = "manual",
	.Run           = ManualPageRun,
};

void RegistermanualPage(void)
{
	RegisterPage(&g_tManualPageAction);
}
